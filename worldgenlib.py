...
class World:
    ...
    def print_uwp(self):
        print(
            f"{self.hex_uwp['starport']}{self.hex_uwp['size']}{self.hex_uwp['atmosphere']}{self.hex_uwp['hydrographics']}{self.hex_uwp['population']}{self.hex_uwp['government']}{self.hex_uwp['law']}-{self.hex_uwp['tl']} {self.base}   {self.zone}      {self.trade_string}  ")
    ...
    def get_world_row(self):
        world_list = [" ", str(self.hex), self.get_uwp_string, self.base, self.trade_string, self.pbg]
        uwp_string = "???????-?"
